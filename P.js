...
function P() {
    ...
    const submitForm = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:5000/api/createprovider', datas);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await axios.get('http://localhost:5000/api/getallprovider')
    ...
    useEffect(()=>{
         axios.get("http://localhost:5000/api//getoneprovider/"+id,datas)
         .then((response)=>{
    ...
    const submiteditFrom=async(e)=>{
        e.preventDefault();
        await  axios.post(`http://localhost:5000/api//updateprovider/${id}`,datas)
        .then((response)=>{
            console.log(response.datas);
        })
    }
        ...
        <div className="container ">
            <div className="crud shadow-lg p-3 mb-5 mt-5 bg-body rounded">
                <div className="row ">
                    ...
                    <div className="col-sm-3 offset-sm-2 mt-5 mb-4 text-gred" style={{ color: "green" }}><h2><b>Provider Details</b></h2></div>
                                    ...
                                    providers.map((providers, index) => {
                                        return (
                                            <tr key={providers._id}>
                                                ...
                                                <td>
                                                    <button className='actionButtons'><i className="bi bi-trash"></i></button>
                                                    <button className='actionButton' onClick={handleShow} onSubmit={submiteditFrom}><i className="bi bi-pen"></i></button>
                                                </td>
                                            </tr>
                                        )
                                    })
